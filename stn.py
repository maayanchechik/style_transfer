...
content_img_pre = utils.content_image_pre_training(imgs[1])
...
#print("style_targets.len()",len(style_targets))
#print("style_targets[0].size()",style_targets[0].size())
...
def pre_train(optimizer, model, nepochs, train_loader,dir_name,saved_test_images_counter):
    pre_train_losses = []
...
################################################################################################
import math
def train_model(optimizer, model, vgg, nepochs, train_loader, alpha, beta, dir_name, reg,saved_test_images_counter):
    '''
    ...
    for e in range(nepochs):
        ...
        for batch_index,(content_batch,y_batch) in enumerate(train_loader):
            ...
            batch_len = len(content_batch)
            content_batch = content_batch.to("cuda")
#            print("content_batch.size() = ",content_batch.size())
	  
            stn_output = model(content_batch) # the outputs of the whole batch
#            print("stn_output.size() = ",stn_output.size())
...
#            print("stn_output_style_features[0].size() = ",stn_output_style_features[0].size())
#            print("stn_output_style_features.len() = ",len(stn_output_style_features))
#            print("stn_output_content_features.size() = ",stn_output_content_features.size())
            
            original_features = utils.get_features(vgg, content_batch)
            original_content_features = original_features[content_layer]
#            print("original_content_features.size() = ",original_content_features.size())
            
            content_loss = utils.calc_content_loss(stn_output_content_features,
                                                   original_content_features)
#            print("after content loss")            
            style_loss = utils.calc_style_loss(style_layers_weights,
                                               stn_output_style_features, style_targets)
#            print("after style loss")
...
# train_loader returns a list of the first batch were the first colomb is the x and the second is the y
# in the x colomb is a tensor of tensors with the size of the batch, where the inner tensors are the
# images in the batch.
...
beta_list = [0.01]
lr_list = [0.001]
#lr_list = [0.00001,0.0001,0.01,0.1]
for beta in beta_list:
    for lr in lr_list:
        name = "_2606_1720_gatys_transforms_batch_size1_a001_b"+str(beta)+"_lr" + str(lr)+"reg"+str(reg)
        ...
        pre_train_losses = pre_train(optimizer, stn, 6, train_loader,
                                     dir_name, saved_test_images_counter)
        ...
        train_losses = train_model(optimizer, stn, vgg, nepochs, train_loader, 0.01, beta, dir_name,reg,saved_test_images_counter)
        print(train_losses)
